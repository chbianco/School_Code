...
% 
% PURPOSE:
% A simple, non-agressive preprocessing pipeline for
...
useSpatialBandpass = true;   % subtract a large-scale background estimate so
                             ...
                             % stationary. Complementary to the temporal
                             % median
...
useNoiseSmoothing = true;    % gently smooth at the particle scale to raise
                             ...
                             % noise. Set NEAR the particle std dev so it acts as
...
absMinThreshold = 10;   % absolute detection-threshold floor, in input-count
                        ...
                        % detecting arbitrarily deep into faint speckle. 
                        % Set it ABOVE your speckle level and
...
for n = 0:num_cams - 1
    ...
    parfor k = 1:nFrames
        stack(:,:,k) = imread(fullfile(inputDir, fileList(k).name));
    end
    ...
    if ~isempty(peakSamples)
        sp        = sort(peakSamples);
        brightRef = sp(max(1, round(0.90 * numel(sp))));
    else
