% Calculate calibration surface for xwire - v1.0  - using polnomial surface
%
...
[Calib_filename,path]=uigetfile('Xwires_Cal*.mat','select the calibration file');
Calib_filename=fullfile(path,Calib_filename);
load(Calib_filename);
...
E1s = reshape(E1_cal(1,:,:),1,Nang_cal*Nspeed_cal);
E2s = reshape(E2_cal(1,:,:),1,Nang_cal*Nspeed_cal);
...
E2min = min(E2s);
E2max = max(E2s);
E1s = (E1s - E1min)/(E1max-E1min);
E2s = (E2s - E2min)/(E2max-E2min);
...
% For now, we simplly do a least-squares fit
...
pgon  = polybuffer(pgon1, .1);  % make a buffer around the voltages
...
grid on
title('Speed [m/s]')
xlabel('E1 [Scaled]'); ylabel('E2 [Scaled]')
...
% filename = sprintf('Xwires_Cal_%s',time_string);
% eval(['save ' filename]);
% fprintf('Calibrataion file saved: %s\n', filename);
