...
%% Mean square displacement and time dependent diffusivity 
dt_track = t(2) - t(1);
max_lag_msd = 2000;
tau_vec_msd = (0:max_lag_msd)' * dt_track;

n_cols = n_bands + 1;   % bands + "all tracks"

% Accumulators
MSD_x = zeros(max_lag_msd + 1, n_cols);
MSD_y = zeros(max_lag_msd + 1, n_cols);
MSD_z = zeros(max_lag_msd + 1, n_cols);
MSD_count = zeros(max_lag_msd + 1, n_cols);

fprintf('\n=== Mean-square displacement ===\n');
fprintf('max_lag = %d (tau_max = %.2f)\n', max_lag_msd, tau_vec_msd(end));
...
for k = 1:nTracks
    ...
    % Compute mean-square displacement at each lag
    n = length(xseg);
    max_lag_this = min(max_lag_msd, n - 1);

    for lag = 0:max_lag_this
        n_pairs = n - lag;

        % Raw displacements
        dx = xseg(1+lag:n_pairs+lag) - xseg(1:n_pairs);
        dy = yseg(1+lag:n_pairs+lag) - yseg(1:n_pairs);
        dz = zseg(1+lag:n_pairs+lag) - zseg(1:n_pairs);

        % Remove mean drift (mean displacement at this lag for this track)
        dx = dx - mean(dx);
        dy = dy - mean(dy);
        dz = dz - mean(dz);

        % Accumulate into "all tracks"
        MSD_x(lag+1, end) = MSD_x(lag+1, end) + sum(dx.^2);
        MSD_y(lag+1, end) = MSD_y(lag+1, end) + sum(dy.^2);
        MSD_z(lag+1, end) = MSD_z(lag+1, end) + sum(dz.^2);
        MSD_count(lag+1, end) = MSD_count(lag+1, end) + n_pairs;

        % Accumulate into specific y-band
        if band_idx > 0
            MSD_x(lag+1, band_idx) = MSD_x(lag+1, band_idx) + sum(dx.^2);
            MSD_y(lag+1, band_idx) = MSD_y(lag+1, band_idx) + sum(dy.^2);
            MSD_z(lag+1, band_idx) = MSD_z(lag+1, band_idx) + sum(dz.^2);
            MSD_count(lag+1, band_idx) = MSD_count(lag+1, band_idx) + n_pairs;
        end
    end
...
MSD_z = MSD_z ./ MSD_count;

% Time-dependent eddy diffusivity: K_i(t) = (1/2) d/dt <dx_i^2>
K_x = zeros(size(MSD_x));
K_y = zeros(size(MSD_y));
K_z = zeros(size(MSD_z));

for b = 1:n_cols
    if MSD_count(1, b) == 0, continue; end
    K_x(:, b) = 0.5 * gradient(MSD_x(:, b), dt_track);
    K_y(:, b) = 0.5 * gradient(MSD_y(:, b), dt_track);
    K_z(:, b) = 0.5 * gradient(MSD_z(:, b), dt_track);
end

% Labels (reuse from autocorrelation if available, otherwise rebuild)
col_labels_msd = cell(1, n_cols);
for b = 1:n_bands
    col_labels_msd{b} = sprintf('$y_0 = %.2f$', y_band_centers(b));
end
col_labels_msd{end} = 'All tracks';

% Print asymptotic diffusivities (average of K over last 20% of window)
fprintf('\n--- Asymptotic eddy diffusivities (from MSD) ---\n');
i_late = round(0.8 * (max_lag_msd + 1)):max_lag_msd + 1;
for b = 1:n_cols
    if MSD_count(1, b) == 0, continue; end
    Kx_asymp = mean(K_x(i_late, b));
    Ky_asymp = mean(K_y(i_late, b));
    Kz_asymp = mean(K_z(i_late, b));
    fprintf('%s: K_x=%.5f, K_y=%.5f, K_z=%.5f\n', ...
            col_labels_msd{b}, Kx_asymp, Ky_asymp, Kz_asymp);
end
...
%% MSD and time dependent diffusivity 
figure;
tiledlayout(1, 3);
...
for b = 1:n_cols
    if MSD_count(1, b) == 0, continue; end
    if b == n_cols
        loglog(tau_vec_msd, MSD_x(:, b), 'k--', 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    else
        loglog(tau_vec_msd, MSD_x(:, b), 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    end
end
% Reference slopes
tau_ref = tau_vec_msd(2:end);
loglog(tau_ref, 1e-3 * tau_ref.^2, 'k:', 'LineWidth', 1, 'DisplayName', '$\sim \tau^2$');
loglog(tau_ref, 2e-2 * tau_ref,    'k-.', 'LineWidth', 1, 'DisplayName', '$\sim \tau$');
xlabel('$\tau$'); ylabel('$\langle \Delta x^2 \rangle$');
title('$\langle \Delta x^2 \rangle$ (streamwise)');
...
for b = 1:n_cols
    if MSD_count(1, b) == 0, continue; end
    if b == n_cols
        loglog(tau_vec_msd, MSD_y(:, b), 'k--', 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    else
        loglog(tau_vec_msd, MSD_y(:, b), 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    end
end
loglog(tau_ref, 1e-3 * tau_ref.^2, 'k:', 'LineWidth', 1, 'DisplayName', '$\sim \tau^2$');
loglog(tau_ref, 2e-3 * tau_ref,    'k-.', 'LineWidth', 1, 'DisplayName', '$\sim \tau$');
xlabel('$\tau$'); ylabel('$\langle \Delta y^2 \rangle$');
title('$\langle \Delta y^2 \rangle$ (wall-normal)');
grid on;
...
for b = 1:n_cols
    if MSD_count(1, b) == 0, continue; end
    if b == n_cols
        loglog(tau_vec_msd, MSD_z(:, b), 'k--', 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    else
        loglog(tau_vec_msd, MSD_z(:, b), 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    end
end
loglog(tau_ref, 1e-3 * tau_ref.^2, 'k:', 'LineWidth', 1, 'DisplayName', '$\sim \tau^2$');
loglog(tau_ref, 5e-3 * tau_ref,    'k-.', 'LineWidth', 1, 'DisplayName', '$\sim \tau$');
xlabel('$\tau$'); ylabel('$\langle \Delta z^2 \rangle$');
title('$\langle \Delta z^2 \rangle$ (spanwise)');
grid on;

% --- Time-dependent eddy diffusivity K_i(t) ---
figure;
tiledlayout(1, 3);
...
for b = 1:n_cols
    if MSD_count(1, b) == 0, continue; end
    if b == n_cols
        plot(tau_vec_msd, K_x(:, b), 'k--', 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    else
        plot(tau_vec_msd, K_x(:, b), 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    end
end
% Add Taylor-formula asymptotes as horizontal lines if available from autocorrelation
if exist('D_u', 'var')
    for b = 1:n_cols
        if b <= n_bands
            yline(D_u(b), ':', 'Color', [0.5 0.5 0.5], 'HandleVisibility', 'off');
        end
    end
end
xlabel('$\tau$'); ylabel('$K_x(\tau)$');
title('$K_x$ (streamwise)');
legend('Location', 'best'); grid on;
...
for b = 1:n_cols
    if MSD_count(1, b) == 0, continue; end
    if b == n_cols
        plot(tau_vec_msd, K_y(:, b), 'k--', 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    else
        plot(tau_vec_msd, K_y(:, b), 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    end
end
if exist('D_v', 'var')
    for b = 1:n_cols
        if b <= n_bands
            yline(D_v(b), ':', 'Color', [0.5 0.5 0.5], 'HandleVisibility', 'off');
        end
    end
end
xlabel('$\tau$'); ylabel('$K_y(\tau)$');
title('$K_y$ (wall-normal)');
grid on;
...
for b = 1:n_cols
    if MSD_count(1, b) == 0, continue; end
    if b == n_cols
        plot(tau_vec_msd, K_z(:, b), 'k--', 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    else
        plot(tau_vec_msd, K_z(:, b), 'LineWidth', 2, 'DisplayName', col_labels_msd{b});
    end
end
if exist('D_w', 'var')
    for b = 1:n_cols
        if b <= n_bands
            yline(D_w(b), ':', 'Color', [0.5 0.5 0.5], 'HandleVisibility', 'off');
        end
    end
end
xlabel('$\tau$'); ylabel('$K_z(\tau)$');
title('$K_z$ (spanwise)');
grid on;

% --- Comparison: Taylor asymptote vs MSD asymptote ---
if exist('D_u', 'var')
    Kx_asymp_vec = zeros(n_bands, 1);
    Ky_asymp_vec = zeros(n_bands, 1);
    Kz_asymp_vec = zeros(n_bands, 1);
    for b = 1:n_bands
        if MSD_count(1, b) == 0, continue; end
        Kx_asymp_vec(b) = mean(K_x(i_late, b));
        Ky_asymp_vec(b) = mean(K_y(i_late, b));
        Kz_asymp_vec(b) = mean(K_z(i_late, b));
    end
    ...
    nexttile;
    plot(y_band_centers, D_u(1:n_bands), 'o-', 'LineWidth', 2, 'DisplayName', 'Taylor: $\sigma^2 T_L$'); hold on;
    plot(y_band_centers, Kx_asymp_vec, 's--', 'LineWidth', 2, 'DisplayName', 'MSD: $K_x(\infty)$');
    xlabel('$y_0$'); ylabel('$D_x$');
    title('Streamwise'); legend('Location', 'best'); grid on;

    nexttile;
    plot(y_band_centers, D_v(1:n_bands), 'o-', 'LineWidth', 2, 'DisplayName', 'Taylor: $\sigma^2 T_L$'); hold on;
    plot(y_band_centers, Ky_asymp_vec, 's--', 'LineWidth', 2, 'DisplayName', 'MSD: $K_y(\infty)$');
    xlabel('$y_0$'); ylabel('$D_y$');
    title('Wall-normal'); grid on;

    nexttile;
    plot(y_band_centers, D_w(1:n_bands), 'o-', 'LineWidth', 2, 'DisplayName', 'Taylor: $\sigma^2 T_L$'); hold on;
    plot(y_band_centers, Kz_asymp_vec, 's--', 'LineWidth', 2, 'DisplayName', 'MSD: $K_z(\infty)$');
    xlabel('$y_0$'); ylabel('$D_z$');
    title('Spanwise'); grid on;
end